...
In the past, I've had great results installing and running [homebrew](https://brew.sh) on behalf of the currently logged in user in order to install a current version via `brew cask install google-chrome` for example. Since macOS 10.14 Mojave, however, this unfortunately no longer practical, as users are prompted by pop-ups to decide whether they want to allow my homebrew automations to "manage" their Mac. In addition, as both macOS and homebrew evolve, and my user-base grows, I want a more boring and stable solution that I can comfortably forget about for a while, so I decided to write my own using cURL.
